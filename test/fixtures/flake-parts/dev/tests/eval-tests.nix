...
rec {
  ...
  packagesNonStrictInDevShells =
    mkFlake
      {
        inputs.self = packagesNonStrictInDevShells;
      }
      {
        systems = [
          "a"
        ...
        config = {
          ...
          flake = {
            modules.generic.example =
              { lib, ... }:
              {
                options.generic.example = lib.mkOption {
                  default = "works in any module system application";
                };
              };
            modules.foo.example =
              { lib, ... }:
              {
                options.foo.example = lib.mkOption {
                  default = "works in foo application";
                };
              };
      ...
      (
        { config, ... }:
        {
          ...
          flake.flakeModules.default =
            { lib, ... }:
            {
              options.flake.test123 = lib.mkOption {
                default = "option123";
              };
    ...
    mkFlake
      ...
      }
      {
        ...
        flake.nixosModules.example =
          { lib, ... }:
          {
            options.test.option = lib.mkOption {
              default = "nixos-test";
            };
          };
      };
  ...
  */
  ...
  dogfoodConsumer =
    mkFlake
      ...
      (
        { flake-parts-lib, ... }:
        {
          imports = [ dogfoodProvider.modules.flake.dogfood ];
        }
      );
  ...
  runTests =
    ...
    assert
      (lib.evalModules {
        class = "barrr";
        modules = [ modulesFlake.modules.generic.example ];
      }).config.generic.example == "works in any module system application";
    assert
      (lib.evalModules {
        class = "foo";
        modules = [ modulesFlake.modules.foo.example ];
    ...
    assert
      (lib.evalModules {
        class = "nixos";
        modules = [ nixosModulesFlake.nixosModules.example ];
