              ...
              if info ? rev then
                ...
                // (
                  if info ? ref then
                    ...
                    { }
                    // (
                      if info ? submodules then
                        {
                          inherit (info) submodules;
                        }
                      else
                        { }
                    )
        ...
        else
          {
            outPath =
              if builtins.isPath src then
                if dirOf (toString src) == builtins.storeDir && builtins ? currentSystem then
...
rec {
  ...
  defaultNix =
    ...
    // (
      if result ? defaultPackage.${system} then
        {
  ...
  shellNix =
    defaultNix
    // (
      if result ? devShell.${system} then
        {
